  ...
  return (
    <div className="group-data-[state=checked]:bg-primary-base group-data-[state=checked]:border-primary-dark h-5 w-5 rounded-sm border bg-white text-white">
      <CheckboxPrimitive.Indicator
        className={cn('flex h-full w-full items-center justify-center text-current', className)}
      >
...
>(({ left, title, description, className, ...props }, ref) => (
  ...
  >
    ...
    <div className="flex flex-1 flex-col gap-1 text-left">
      <div>
        <span className="text-main group-disabled:text-sub text-label-sm">{title}</span>
      </div>
      <p className="text-p-sm group-disabled:text-soft text-sub">{description}</p>
    </div>
    <Checkbox />
  </CheckboxPrimitive.Root>
));
